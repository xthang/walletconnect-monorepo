...
export class WalletClient {
  ...
  static async init(provider: UniversalProvider, opts: Partial<WalletClientAsyncOpts>): Promise<WalletClient> {
  ...
  private getWallet(privateKey?: string) {
    const wallet = typeof privateKey !== 'undefined' ? new ethers.Wallet(privateKey) : ethers.Wallet.createRandom()
    ...
    this.client.on('session_request', async (requestEvent: SignClientTypes.EventArguments['session_request']) => {
      ...
      try {
        ...
        if (parseInt(chainRef, 10) !== this.chainId) {
          throw new Error(`Target chainId (${chainRef}) does not match active chainId (${this.chainId})`)
        }

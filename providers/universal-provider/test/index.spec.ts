...
import { ERC20Token__factory, _abi, _bytecode } from 'ethereum-test-network/lib/utils/ERC20Token__factory'
...
describe('UniversalProvider', function () {
  ...
  describe('eip155', () => {
    ...
    describe('Web3', () => {
      ...
      it('ERC20 contract', async () => {
        ...
        const transferTx = erc20.methods.transfer(receiverAddress, utils.parseEther('1').toHexString())
      ...
      it('send transaction', async () => {
        ...
        expect(
          ...
          'balanceAfter ' + balanceAfter.toString() + ' less than balanceBefore: ' + balanceBefore.toString()
        ).to.be.true
      })
    ...
    describe('Ethers', () => {
      ...
      it('ERC20 contract', async () => {
        ...
        const tokenTransferGas = await erc20.estimateGas.transfer(receiverAddress, utils.parseEther('1'))
        expect(tokenTransferGas.toString()).to.eql('52437')
  ...
  describe('cosmos', () => {
    it('should sign cosmos_signDirect request', async () => {
      ...
      const valid = await verifyDirectSignature(await walletClient.cosmosWallet.getAddress(), result.signature, signDoc)
